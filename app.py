...
def format_items(items, category):
    ...
    formatted_items = []
    for item in items:
        item_icon_url = item['Items Icon']
        item_id = item['Items ID']
        
        # Adding item with image directly in the embed
        formatted_items.append(f"**Item ID: {item_id}**\n{item_icon_url}")  # Just showing the URL, Discord will render it as an image
...
async def get(ctx, uid: str):
    ...
    outfit_items = equipped_items.get('Equipped Outfit', [])
    weapon_items = equipped_items.get('Equipped Weapon', [])
